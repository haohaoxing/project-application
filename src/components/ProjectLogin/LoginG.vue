...
<template>
  <div>
      <form name="LoginG" class="loginContent" id="ShenHeForm">
        <h1>管理员登录</h1>
        <p>
            <label>账户</label>
        ...
        <p>
            <input class="sub" @click="TiJiao2()" id="sub2" type="submit"/>
        </p>
    </form>
  </div>
</template>
     ...
     checked(){
       ...
       TiJiao2(){
         console.log($("#userName").val(),$("#userPass").val());
            if($("#userName").val()=="admin"&&$("#userPass").val()=="888888"){
                location.href="http://localhost:8080/#/Administratoraudit";
            }else{
                alert("用户名/密码错误");
            }
          }	
